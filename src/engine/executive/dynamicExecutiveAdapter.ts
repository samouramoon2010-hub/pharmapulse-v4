...
// Only consumed by dynamicExecutiveScore.ts → dynamicExecutiveDataPath.ts
// (itself unwired) and this module's own dedicated test files. Retained
// as dormant legacy/foundation code, not deleted, because it has
// meaningful test coverage of its own. Do not activate. The isCore
// gates in validateExecutiveKpiWeights() and getExecutiveEnabledKpis()
// below are real but currently unreachable from any active production
// path — left as-is pending a future decision to activate this engine.
//
