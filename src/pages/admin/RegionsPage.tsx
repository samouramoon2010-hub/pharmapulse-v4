...
const EMPTY: Partial<Region> = { code: '', name: '', managerUid: '', active: true }
...
export default function RegionsPage() {
  ...
  // Phase 1B-2B: active regional_manager users — loaded once on mount
  const [managers,  setManagers]  = useState<Array<{ uid: string; displayName: string }>>([])

  useEffect(() => {
    const u = subscribe()
    // One-time fetch — manager list changes rarely during an admin session
    getDocs(query(
  ...
  // uid → displayName lookup used by the table and the dropdown
  const managerMap = useMemo(
    () => Object.fromEntries(managers.map((m) => [m.uid, m.displayName])),
    [managers],
  )
  ...
  const openEdit   = (r: Region) => {
    setForm({
      code:       r.code,
      name:       r.name,
      managerUid: r.managerUid || '',  // '' = "no manager" sentinel in the form select
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    // Normalise managerUid: empty-string sentinel → null before persisting
    const payload = { ...form, managerUid: form.managerUid || null }
    try {
      if (editId) {
  ...
  const COLS = [
    ...
    { key: 'name', label: 'Name', sortable: true, render: (v, row: Region) => (
      <div>
        <div style={{ fontSize: '12px', color: 'var(--text-primary)' }}>{v}</div>
        <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '1px' }}>
          {managerMap[row.managerUid] || 'No manager'}
        </div>
      </div>
    )},
      ...
      {showForm && (
        <div style={{
          ...
          <div style={{
            ...
            <F label="Manager" error={undefined}>
              <select
                value={form.managerUid || ''}
                onChange={(e) => sf('managerUid', e.target.value)}
                style={{ width: '100%', height: '34px', fontSize: '12px' }}
              >
              ...
              {managers.length === 0 && (
                <p style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '4px' }}>
                  No active regional managers found — create users with the Regional Manager role first.
                </p>
              )}
            </F>
