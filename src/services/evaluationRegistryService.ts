...
export async function publishEvaluationProfile(
  ...
  // Profile Integrity Guard — registry-aware check, run AFTER the structural
  // validation above. Blocks publish for unknown/inactive KPIs, invalid
  // weights, empty/unreachable baskets, and missing threshold rules.
  // Cross-basket KPI reuse is reported as a warning only (confirmed
  // intentional, supported pattern) — never blocks publish.
  const { checkProfileIntegrity } = await import('../engine/evaluationRegistry/profileIntegrityGuard')
  const { fetchKpiRegistryOnce }  = await import('./kpiRegistryService')
  const registry  = await fetchKpiRegistryOnce()
  const integrity = checkProfileIntegrity(current, registry)
  if (!integrity.valid) {
    throw new Error(`Profile integrity check failed: ${integrity.errors.map((e) => e.message).join('; ')}`)
  }
  ...
  await logAction({
    ...
    before:     { status: 'draft' },
    after:      {
      status: 'published', name: current.name, version: current.version,
      crossBasketKpiReuseCount: integrity.crossBasketKpiUsage.length,
    },
  })
}
