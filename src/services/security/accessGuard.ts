...
// Phase 1A: 'general_manager' added — route access only; data scope is Phase 2.
export type UserRole =
  | 'admin'
  | 'manager'             // alias — kept for existing users
  | 'branch_manager'
  | 'district_supervisor' // Phase 1A: route access; data scope Phase 2
  | 'regional_manager'    // Phase 1A: route access; data scope Phase 2
  | 'general_manager'     // Phase 1A: route access; data scope Phase 2
  | 'pharmacist'
...
/**
 * Phase 1A: returns true for general_manager.
 * Tenant-wide read-only role. Data scope enforcement is Phase 2.
 */
export function isGeneralManager(ctx: GuardContext): boolean {
  return ctx.role === 'general_manager'
}
