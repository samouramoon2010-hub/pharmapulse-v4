...
function F({ label, required = false, error, children, hint }: {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>
            ...
            <F label="Role" required>
              <select value={draft.role as string} disabled={readOnly}
                onChange={(e) => setField('role', e.target.value)} style={INP}>
...
export default function EvaluationRegistryPage() {
  ...
  const kpiOptions = useMemo(() =>
    getActiveKpis(liveRegistry).map((k) => ({ key: k.key, label: k.label })),
    [liveRegistry]
  )
