...
export default function RegionalIntelligencePanel({ intelligence, branchRollups = [], liveRegistry, isAdmin = false, onSendCriticalRiskAlert, sendingAlert = false }) {
  ...
  // registry-driven so dynamic/custom production KPIs appear as columns
  // automatically once aggregated by the regional engine.
  const kpiColKeys   = getProductionEngineKeys(liveRegistry ?? DEFAULT_KPI_REGISTRY)
      ...
      {open && (
        <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ marginTop: '12px', borderTop: '1px solid var(--border-subtle)', paddingTop: '12px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                ...
                >
                  ...
                  {heatmapOpen
                    ? <ChevronUp style={{ width: 11, height: 11, color: 'var(--text-muted)' }} />
                    : <ChevronDown style={{ width: 11, height: 11, color: 'var(--text-muted)' }} />
                  }
                </button>
          ...
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', textAlign: 'right' }}>
            Generated {new Date(intelligence.generatedAt).toLocaleTimeString('en-US')}
          </div>
        </div>
      )}
    </div>
