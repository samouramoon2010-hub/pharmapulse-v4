...
export interface UserTerritoryFields {
  ...
  // Phase 1B: cache fields — populated by recomputeAssignedPharmacyIds() in Phase 1B-3.
  // Stored on user documents but never read by runtime logic until Phase 2 scope resolver.
  // null = sentinel for "all access" (admin/GM) or "not applicable" (branch/pharmacist roles)
  // []   = assigned role but no pharmacies in territory yet
  assignedPharmacyIds?: string[] | null  // query cache for multi-branch scope (supervisor/manager)
  assignedDistrictIds?: string[]         // forward-compat: reserved for future large-territory batching
}
