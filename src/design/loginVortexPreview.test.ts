// ============================================================
// Login Vortex Preview — Certification
//
// Scope: an isolated preview route (/login-vortex-preview) pairing a
// cropped, text-free portion of the rejected Login V3 reference
// artwork (design-reference/login-v3-reference.png) — composited as
// an inset graphic on a flat canvas, not stretched full-bleed — with
// a real-structure React login panel. Local-only form state — no
// auth wiring, no Firestore/Auth change. Confirms production /login
// and the prior /login-network-preview are untouched.
//
// See docs/production/LOGIN_VORTEX_PREVIEW.md.
// ============================================================
import { describe, it, expect } from 'vitest'

const appSrc      = await import('../App.jsx?raw').then((m) => m.default)
const previewSrc   = await import('../pages/auth/LoginVortexPreview.jsx?raw').then((m) => m.default)
const networkSrc   = await import('../pages/auth/LoginNetworkPreview.jsx?raw').then((m) => m.default)
...
// The header doc-comment explains why the original V3 reference image
// (which DID bake in Face ID/Passkey buttons) was rejected — that
// explanatory mention is provenance documentation, not a feature in
// this component. Scope the "no biometric UI" check to the actual
// code, starting at the first import.
const previewCode = previewSrc.slice(previewSrc.indexOf('import React'))

// ════════════════════════════════════════════════════════════
// 1 — Route is additive; production /login and the prior preview are untouched
// ════════════════════════════════════════════════════════════
describe('1 — preview route is additive; production /login is untouched', () => {
  it('the /login-vortex-preview route is registered', () => {
    expect(appSrc).toContain('path="/login-vortex-preview"')
    expect(appSrc).toContain('LoginVortexPreview')
  })
  ...
  it('the prior /login-network-preview route is still present and unmodified', () => {
    expect(appSrc).toContain('path="/login-network-preview"')
    expect(networkSrc).not.toContain('LoginVortexPreview')
  })
  ...
  it('LoginPageV2.jsx (production /login) source is byte-for-byte unaffected by this preview', () => {
    ...
    expect(loginV2Src).not.toContain('LoginVortexPreview')
  })

  it('authStore.js was not modified by this preview', () => {
    expect(authStoreSrc).toContain('AUTH_ERROR_MAP')
    expect(authStoreSrc).not.toContain('LoginVortexPreview')
  })
})
...
describe('2 — preview is visual-only, no auth service connection', () => {
  ...
  it('has no real submit logic (preventDefault only, simulated local status, no auth call)', () => {
    ...
    const submitFnBody = previewSrc.slice(submitFnStart, previewSrc.indexOf('const bgSrc', submitFnStart))
...
describe('3 — no personal data, no credential exposure, no biometric capability', () => {
  ...
  it('has no Face ID / Passkey / WebAuthn reference in the actual component code', () => {
    expect(previewCode).not.toMatch(/face\s*id/i)
    expect(previewCode).not.toMatch(/passkey/i)
    expect(previewCode).not.toMatch(/webauthn/i)
    expect(previewCode).not.toMatch(/navigator\.credentials/)
    expect(previewCode).not.toMatch(/PublicKeyCredential/)
  })
...
// ════════════════════════════════════════════════════════════
// 4 — Background is a static inset composite, not a stretched/destructive crop
// ════════════════════════════════════════════════════════════
describe('4 — background asset is an inset composite, swapped by breakpoint', () => {
  it('references the desktop and mobile vortex composite file stems', () => {
    expect(previewSrc).toContain("'/login-vortex-preview/login-bg-vortex-'")
  ...
  it('uses a plain <img> (the source is a static composite, not a video/animation)', () => {
    expect(previewSrc).toMatch(/<img[^>]*bgSrc/)
    expect(previewSrc).not.toContain('<video')
  })
  it('the background image is decorative (aria-hidden, empty alt)', () => {
    expect(previewSrc).toMatch(/aria-hidden="true"/)
    expect(previewSrc).toMatch(/alt=""/)
  })
})
...
describe('5 — fully labeled, accessible, semantic form', () => {
  ...
  it('has a prepared error live region and a prepared loading state', () => {
...
// ════════════════════════════════════════════════════════════
// 6 — Desktop compact panel + mobile scroll-safe structure
// ════════════════════════════════════════════════════════════
describe('6 — desktop compact panel + mobile scroll-safe structure (no overlap with the artwork)', () => {
  ...
  it('panel height is content-driven (no fixed/full-height card rule)', () => {
    expect(previewSrc).not.toMatch(/\.lvp-panel\s*\{[^}]*height:\s*100vh/)
    expect(previewSrc).not.toMatch(/\.lvp-panel\s*\{[^}]*height:\s*calc\(100vh/)
  })
  it('mobile anchors the panel below the artwork with a fixed offset, not viewport-relative centering', () => {
    expect(previewSrc).toMatch(/padding-top:\s*57vh/)
  })
  it('the background uses position:fixed so it still covers the viewport while mobile content scrolls', () => {
    expect(previewSrc).toMatch(/\.lvp-bg\s*\{[^}]*position:\s*fixed/)
  })
  it('the root allows vertical scroll (only overflow-x is constrained)', () => {
    expect(previewSrc).toMatch(/overflow-x:\s*hidden/)
    expect(previewSrc).not.toMatch(/\.lvp-root\s*\{[^}]*overflow:\s*hidden/)
  })
...
describe('8 — direction-aware styling via the real app locale, stable composition', () => {
  it('uses the real useI18n hook for dir/lang rather than hardcoding a locale', () => {
    ...
    expect(previewSrc).not.toMatch(/dir="ltr"[\s>]/)
  })
  ...
  it('pins the macro layout (artwork side + panel side) with an explicit CSS direction, not a physical/locale mirror', () => {
...
// 9 — No new dependency
// ════════════════════════════════════════════════════════════
describe('9 — no new dependency, no canvas/WebGL/particle library', () => {
