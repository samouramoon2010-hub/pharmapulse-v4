...
describe('Firestore rules audit — kpi_entries', () => {
  it('read rule is scoped to own entries, own-branch, and admin — standalone isMgr() removed (Hardening Fix 3)', async () => {
    const src = await import('../../../firestore.rules?raw')
    const block = src.default.split('match /kpi_entries/')[1]?.split('match /')[0] ?? ''
    const readBlock = block.split('allow read:')[1]?.split('allow create:')[0] ?? ''
    // ownsPharmacy (branch-scoped manager) and own-userId must still be present
    expect(readBlock).toContain('ownsPharmacy(resource.data.pharmacyId)')
    expect(readBlock).toContain('resource.data.userId == uid()')
    // standalone isMgr() removed — it allowed cross-branch reads for any manager
    expect(readBlock).not.toContain('isMgr()')
  })
