...
function scoreElement(
  ...
  // Threshold band
  const band = matchThresholdBand(achievementPct, thresholdRule)
...
function scoreBasket(
  ...
  // Weighted average achievement across elements
  const aggregateAchievementPct = elementResults.reduce(
    (sum, el) => sum + el.achievementPct * el.weight,
    0,
  )
...
export function runEvaluation(input: EvaluationEngineInput): EvaluationResult {
  ...
  const basketResults = activeBaskets.map((basket) =>
    scoreBasket(basket, input, missingKpis)
  )
