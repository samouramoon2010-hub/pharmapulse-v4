...
/**
 * Real-time listener: all PUBLISHED personal targets for a branch+month.
 *
 * Used by TeamPage to populate PharmacistInput.personalTarget for each
 * team member. Only published targets are passed — drafts are excluded
 * so that a manager's in-progress allocation does not affect Team
 * Intelligence calculations until they explicitly publish it.
 *
 * Firestore index required: pharmacyId + month + status (ASCENDING each)
 */
export function subscribePublishedPersonalTargetsByBranch(
  pharmacyId: string,
  month:      string,
  callback:   (docs: PersonalTargetDoc[]) => void,
): () => void {
