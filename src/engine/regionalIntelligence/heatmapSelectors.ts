...
/**
 * Determine if a KPI key is a core engine key.
 * Core keys are always aggregated by the regional engine.
 * Non-core keys (custom) may not be → NOT_AGGREGATED.
 */
function isCoreKpiKey(key: string): boolean {
  return (KPI_KEYS as readonly string[]).includes(key)
}
...
function resolveKpiCell(
  ...
  // Case 2: custom KPI — not in core engine keys → NOT_AGGREGATED
  if (!isCoreKpiKey(kpiKey)) {
    return { value: null, status: null, emptyCellState: 'NOT_AGGREGATED' }
  }

  // Case 3: core KPI — look up the rollup summary
  const kpiSummary = findKpiSummary(branch, kpiKey)
  if (!kpiSummary) {
    // Core KPI but somehow missing from summary — should not happen in practice
    return { value: null, status: null, emptyCellState: 'NO_DATA' }
  }
