...
// ── PR-1B: secure random password ─────────────────────────────
// Used when the caller (UsersPage) does not supply a password —
// the admin never sees, types, or transmits the account's real
// password. The account is created with this throwaway value and
// a reset email is sent immediately so the user sets their own.
// Never logged, stored, or returned to any caller.
function generateSecurePassword() {
  const bytes = crypto.getRandomValues(new Uint8Array(24))
  const b64   = btoa(String.fromCharCode(...bytes)).replace(/[+/=]/g, '')
  return `${b64.slice(0, 28)}Aa1!` // satisfies length/charset; never displayed
}
...
export async function createUser({
  ...
  // PR-1B: no admin-entered temporary password. When the caller omits
  // password entirely, generate one internally and force the reset email —
  // the admin never knows the account's actual password, and there is no
  // way to sign in without going through the reset flow first.
  const isAutoGeneratedPassword = !password
  const effectivePassword = password || generateSecurePassword()
  const mustSendWelcomeEmail = sendWelcomeEmail || isAutoGeneratedPassword
  ...
  // Step 1: Auth
  const uid = await createFirebaseAuthUser(email, effectivePassword)
...
// Used by the Personal Targets page to build the allocation table, and
// (via DashboardPage/TeamPage/useBranchIntelligenceData/
// useExecutiveTeamRollup/EvaluationRunPage/bulkEvaluationService) as
// the single central roster boundary for nearly every "pharmacists at
// this branch" reader in the app.
//
// Closure Patch Part 2: a CLAIMED pending-onboarding document (the
// pre-activation `pending_${employeeId}` doc, soft-archived after
// activatePendingPharmacist() claims it — see
// services/dataExchange/pharmacistActivationService.ts) keeps its
// original `active`/`pharmacyId` fields untouched by the claim
// transaction, so without this exclusion it would still match this
// query's filters and appear ALONGSIDE the new real Auth-linked
// document for the same pharmacist — a silent double-count. CLAIMED
// docs are historical identity-link artifacts only; they must never
// be filtered back in here. The doc itself is never deleted — only
// excluded from this operational roster.
