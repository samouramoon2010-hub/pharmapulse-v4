...
export async function updateRegion(
  ...
  // ── Territory sync — Phase 1B-3C ─────────────────────────────

  // Best-effort: recompute assignedPharmacyIds cache when region manager changes.
  // Failure is logged but must not fail the region update itself.
  const oldManagerUid = (before?.managerUid as string | null | undefined) || null
  const newManagerUid = ('managerUid' in data)
    ? ((data.managerUid as string | null | undefined) || null)
    : oldManagerUid
  if (oldManagerUid !== newManagerUid) {
    if (oldManagerUid) {
      recomputeAssignedPharmacyIds(oldManagerUid, actorId, actorRole).catch((e) =>
        console.error('[regionService] recompute old manager failed', e),
      )
    }
    if (newManagerUid) {
      recomputeAssignedPharmacyIds(newManagerUid, actorId, actorRole).catch((e) =>
        console.error('[regionService] recompute new manager failed', e),
      )
    }
  }
}
